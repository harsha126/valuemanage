...
public class ReportServiceImpl implements ReportService {
    ...
    public Report save(NewReport newReport) throws ParseException {
        SimpleDateFormat format = new SimpleDateFormat("dd-MM-yyyy");
        Comment comment = Comment.builder().text(newReport.getText()).name(newReport.getName()).date(newReport.getCommentDate()).build();
        Comment savedComment = commentService.save(comment);
        Report report = Report.builder().metNew(newReport.getMetNew()).metOld(newReport.getMetOld()).metTotal(newReport.getMetTotal()).orders(newReport.getOrders()).comment(savedComment).date(newReport.getReportDate()).build();
        report.setDate(format.parse(format.format(report.getDate())));
        return reportRepository.save(report);
    }
}
